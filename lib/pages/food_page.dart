...
class SelectFoodPageState extends State<SelectFoodPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if(foodBox.length > 0){
      foods = foodBox.values.toList();
    }else{
      var ex = "Error: no food";
      throw ex;
    }
    return Scaffold(
    ...
    return Container(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          String food = foods.elementAt(index).name;
          String kCal = foods.elementAt(index).kCal.toString();
          return Card(
              child: ListTile(
                title: Text(kCal+" kCal"+" "+food),
                onTap: () { //
                  selected.add(foods.elementAt(index));
                  if(mounted) {
                    setState(() {
                    var confirm = popUp();
